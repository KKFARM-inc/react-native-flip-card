...
export default class FlipCard extends Component {
  ...
  componentWillReceiveProps (nextProps) {
    if (this.state.isFlipped !== nextProps.flip && !this.state.isFlipping) {
      this._toggleCard(nextProps.swipeDirection)
    }
  }
  _toggleCard (swipeDirection) {
    this.setState({isFlipping: true})
    this.props.onFlipStart(this.state.isFlipped)
    this._animation(!this.state.isFlipped,swipeDirection)
  }
  _animation (isFlipped,swipeDirection) {
    let directionValue = swipeDirection == 'right'? this.rotateValue + 1 : this.rotateValue - 1;
    if (!this.timer) {
    ...
    Animated.spring(this.state.rotate,
     {
      ...
      if(!this.state.isFlipped) {
        ...
        this.rotateValue = directionValue
      }
